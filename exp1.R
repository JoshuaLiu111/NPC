...
save(roll_50_50,roll_100_50,roll_150_50,roll_200_50,roll_250_50,roll_300_50,
     ...
     file = 'linear4features010.Rdata')

################################################
load('linear4features005.Rdata')
load('linear4features010.Rdata')


#histogram_300_200
hist_300_200<-melt(roll_300_200[[3]][,1:2])
ggplot(hist_300_200, aes(x=value, color=variable)) +
  scale_color_manual(values=c("#E69F00","#999999","#56B4E9"))+
  geom_histogram(fill="white", alpha=0.5, position="dodge",bins=25)+
  xlab("Profit") + ylab("Frequency") + labs(color = "Methods")+
  geom_segment(aes(x = roll_300_200[[1]][2], y = 0, xend = roll_300_200[[1]][2], 
                   yend = 50),linetype="dashed",color="#999999")+
  geom_segment(aes(x = roll_300_200[[1]][1], y = 0, xend = roll_300_200[[1]][1], 
                   yend = 50),linetype="dashed",color="#E69F00")+
  geom_segment(aes(x = roll_300_200[[1]][3], y = 0, xend = roll_300_200[[1]][3], 
                   yend = 50),linetype="dashed",color="darkred")+
  theme(text = element_text(size = 20)) 
ggsave("npsa_hist.pdf", width = 8, height = 5)
...
ggsave("draw_dl_x_200.pdf", width = 8, height = 5)
...
ggplot(draw_dl_300_y,aes(iteration,value,col=variable))+
  ...
  geom_point(shape=4,size=3)+
  theme(text = element_text(size = 20)) 
ggsave("draw_dl_300_y.pdf", width = 8, height = 5)
