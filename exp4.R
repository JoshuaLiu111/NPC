...
get_cvar<-function(proportion_series,par){
  ...
  cum<-c()
  for (i in 1:m) {
    exceed_loss<-max(c(-profit_function_linear(order[i],proportion_series[i,5])-par[6],0))
...
get_proportion<-function(series, beta){
  ...
  timeseries<-msts(series[,ncol(series)],seasonal.periods = c(4,12))
...
test_rolling<-function(series,trainsize,rollnum){
  ...
  for (j in 1:rollnum) {
    ...
    temp.sqr<-rq(demand_series ~ .,data=train_series,tau=(1-beta)*U/(E+U))$coefficients*((E+W)/(E+U))+
      rq(demand_series ~ .,data=train_series,tau=(E*beta+U)/(E+U))$coefficients*((U-W)/(E+U))
    ...
    value.temp.nf<-as.numeric(forecast(temp.nf,1)$mean+get_sol_residual(temp.nf$residuals,beta))
    ...
    value.temp.plm<-as.numeric(temp.plm$coefficients[1]+temp.plm$coefficients[2]*test_series[1]+
                                 ...
                                 temp.plm$coefficients[5]*test_series[4]+get_sol_residual(temp.plm$residuals,beta))
    ...
    record.temp.npc<-c(record.temp.npc,profit_function_linear(value.temp.npc,test_series[5]))
    record.temp.sa<-c(record.temp.sa,profit_function_linear(value.temp.sa,test_series[5]))
    record.temp.um<-c(record.temp.um,profit_function_linear(value.temp.um,test_series[5]))
    record.temp.nf<-c(record.temp.nf,profit_function_linear(value.temp.nf,test_series[5]))
    record.temp.sqr<-c(record.temp.sqr,profit_function_linear(value.temp.sqr,test_series[5]))
    record.temp.plm<-c(record.temp.plm,profit_function_linear(value.temp.plm,test_series[5]))
  }
...
x_1<-sim.ssarima(frequency=12,orders=list(ar=c(1,1,1),ma=c(1,1,1)),lags=c(1,4,12),AR=c(0.2,0.2,0.1),MA=c(0.1,0.2,0.1),obs = 500,constant = 1000,mean=0,sd=200)$data
x_2<-sim.ssarima(frequency=12,orders=list(ar=c(2,1,1),ma=c(1,1,0)),lags=c(1,4,12),AR=c(0.1,0.1,0.2,0.1),MA=c(0.1,0.1),obs = 500,constant = 600,mean=0,sd=200)$data
x_3<-sim.ssarima(frequency=12,orders=list(ar=c(1,2,0),ma=c(2,0,1)),lags=c(1,4,12),AR=c(0.1,0.2,0.1),MA=c(0.2,0.1,0.1),obs = 500,constant = 300,mean=0,sd=50)$data
x_4<-sim.ssarima(frequency=12,orders=list(ar=c(1,1,0),ma=c(0,0,1)),lags=c(1,4,12),AR=c(0.1,0.2),MA=c(0.1),obs = 500,constant = 1000,mean=0,sd=100)$data
...
save(roll_50_50,roll_100_50,roll_150_50,roll_200_50,roll_250_50,roll_300_50,
     ...
     file = 'linear4features005_ins1.Rdata')
#############################
load('linear4features005_ins1.Rdata')
dl_50_y<-data.frame(NPC=c(roll_50_50[[1]][1],roll_50_100[[1]][1],roll_50_150[[1]][1],
                           roll_50_200[[1]][1]),
                     SA=c(roll_50_50[[1]][2],roll_50_100[[1]][2],roll_50_150[[1]][2],
                          roll_50_200[[1]][2]),
                     UM=c(roll_50_50[[1]][3],roll_50_100[[1]][3],roll_50_150[[1]][3],
                          roll_50_200[[1]][3]),
                     NF=c(roll_50_50[[1]][4],roll_50_100[[1]][4],roll_50_150[[1]][4],
                          roll_50_200[[1]][4]),
                     SQR=c(roll_50_50[[1]][5],roll_50_100[[1]][5],roll_50_150[[1]][5],
                           roll_50_200[[1]][5]),
                     PLM=c(roll_50_50[[1]][6],roll_50_100[[1]][6],roll_50_150[[1]][6],
                           roll_50_200[[1]][6]))

relative_dl_50_y<-data.frame(NPC=(dl_50_y[,1]-dl_50_y[,2])/(dl_50_y[,3]-dl_50_y[,2]),
                              NF=(dl_50_y[,4]-dl_50_y[,2])/(dl_50_y[,3]-dl_50_y[,2]),
                              SQR=(dl_50_y[,5]-dl_50_y[,2])/(dl_50_y[,3]-dl_50_y[,2]),
                              PLM=(dl_50_y[,6]-dl_50_y[,2])/(dl_50_y[,3]-dl_50_y[,2]),
...
x_1<-sim.ssarima(frequency=12,orders=list(ar=c(1,1),ma=c(1,1)),lags=c(1,12),AR=c(0.1,0.2),MA=c(0.1,0.2),obs = 500,constant = 1000,mean=0,sd=200)$data
x_2<-sim.ssarima(frequency=12,orders=list(ar=c(2,1),ma=c(1,0)),lags=c(1,12),AR=c(0.1,0.1,0.2),MA=c(0.1),obs = 500,constant = 600,mean=0,sd=200)$data
x_3<-sim.ssarima(frequency=12,orders=list(ar=c(1,0),ma=c(0,1)),lags=c(1,4),AR=c(0.2),MA=c(0.1),obs = 500,constant = 300,mean=0,sd=50)$data
x_4<-sim.ssarima(frequency=12,orders=list(ar=c(1,1),ma=c(1,0)),lags=c(1,4),AR=c(0.1,0.2),MA=c(0.1),obs = 500,constant = 1000,mean=0,sd=100)$data
be<-runif(4,0.25,0.75)
demand_series<-ts(be[1]*x_1+be[2]*x_2+be[3]*x_3+be[4]*x_4+rnorm(500,0,100)+
                    rlaplace(500,0,71)+runif(500,-50,50),start = c(1,1),frequency = 12)
series<-data.frame(x_1,x_2,x_3,x_4,demand_series)
...
save(roll_50_50,roll_100_50,roll_150_50,roll_200_50,roll_250_50,roll_300_50,
     ...
     roll_50_150,roll_100_150,roll_150_150,roll_200_150,roll_250_150,roll_300_150,
     roll_50_200,roll_100_200,roll_150_200,roll_200_200,roll_250_200,roll_300_200,
     file = 'linear4features005_ins2.Rdata')
#############################
load('linear4features005_ins2.Rdata')
dl_50_y<-data.frame(NPC=c(roll_50_50[[1]][1],roll_50_100[[1]][1],roll_50_150[[1]][1],
                          roll_50_200[[1]][1]),
                    SA=c(roll_50_50[[1]][2],roll_50_100[[1]][2],roll_50_150[[1]][2],
                         roll_50_200[[1]][2]),
...
x_1<-sim.ssarima(frequency=12,orders=list(ar=c(1,1),ma=c(1,1)),lags=c(1,12),AR=c(0.1,0.2),MA=c(0.1,0.1),obs = 500,constant = 1000,mean=0,sd=200)$data
x_2<-sim.ssarima(frequency=12,orders=list(ar=c(2,1),ma=c(1,0)),lags=c(1,12),AR=c(0.2,0.1,0.2),MA=c(0.1),obs = 500,constant = 600,mean=0,sd=200)$data
x_3<-sim.ssarima(frequency=12,orders=list(ar=c(1,1),ma=c(0,1)),lags=c(1,12),AR=c(0.1,0.2),MA=c(0.2),obs = 500,constant = 300,mean=0,sd=50)$data
x_4<-sim.ssarima(frequency=12,orders=list(ar=c(1,1),ma=c(1,0)),lags=c(1,4),AR=c(0.1,0.2),MA=c(0.1),obs = 500,constant = 1000,mean=0,sd=100)$data
be<-runif(4,0.25,0.75)
demand_series<-ts(be[1]*x_1+be[2]*x_2+be[3]*x_3+be[4]*x_4+rnorm(500,0,100)+
                    rlaplace(500,0,71)+runif(500,-50,50),start = c(1,1),frequency = 12)
...
save(roll_50_50,roll_100_50,roll_150_50,roll_200_50,roll_250_50,roll_300_50,
     ...
     roll_50_150,roll_100_150,roll_150_150,roll_200_150,roll_250_150,roll_300_150,
     roll_50_200,roll_100_200,roll_150_200,roll_200_200,roll_250_200,roll_300_200,
     file = 'linear4features005_ins3.Rdata')
#############################
load('linear4features005_ins3.Rdata')
dl_50_y<-data.frame(NPC=c(roll_50_50[[1]][1],roll_50_100[[1]][1],roll_50_150[[1]][1],
                          roll_50_200[[1]][1]),
                    SA=c(roll_50_50[[1]][2],roll_50_100[[1]][2],roll_50_150[[1]][2],
                         roll_50_200[[1]][2]),
